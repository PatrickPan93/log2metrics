...
func Run(ctx context.Context, rate int) {
	...
	f, _ := os.OpenFile("access.log", os.O_WRONLY|os.O_CREATE|os.O_SYNC|os.O_APPEND, 0755)
	...
	for {
		...
		case <-ctx.Done():
			f.Close()
			log.Println("common.nginxLogGenerator.receive_quit_signal_and_quit")
			return
		}
	}
}
